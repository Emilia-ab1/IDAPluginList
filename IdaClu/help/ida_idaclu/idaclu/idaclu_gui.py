...
class IdaCluDialog(qt_shims.get_QWidget()):
    def __init__(self, env_desc):
        ...
        self.ui.PaletteTool.setClickHandler(self.changeFuncColor)
    ...
    def bindClicks(self):
        ...
        self.ui.LabelTool.setLabelHandler(self.toggleModeMerge)  # feat_folders
        self.ui.LabelTool.setModeHandler(self.toggleRecursion)
        self.ui.LabelTool.setSetHandler(self.addMerge)
        self.ui.LabelTool.setClsHandler(self.clsMerge)

    def applyStyling(self):
        self.ui.LabelTool.ModeToggle.setProperty('class','tool-btn tool-btn-hov')
        self.ui.LabelTool.LabelEdit.setProperty('class','head-edit')
        self.ui.LabelTool.SetLabelButton.setProperty('class','tool-btn tool-btn-hov')
        self.ui.LabelTool.ClsLabelButton.setProperty('class','tool-btn tool-btn-hov')
    ...
    def viewSelChanged(self):
        self.ui.LabelTool.setEnabled(True)
        self.ui.PaletteTool.setEnabled(True)

    def initPrefixFilter(self):
        self.ui.PrefixFilter.addItems(self.prefixes)
        self.ui.PrefixFilter.setText("")

    def initFoldersFilter(self):
        if self.env_desc.feat_folders:
            if len(self.folders) == 0:
                self.ui.FolderFilter.setEnabled(False)
                # leave "as-is"
                # do not populate the corresponding combo-box
            else:
                # leave "as-is"
                # do not disable the corresponding combo-box
                self.ui.FolderFilter.addItems(self.folders)

            # do not print initial value
            self.ui.FolderFilter.setText("")
            # default pointing cursor and
            # hoverable button
            self.ui.LabelTool.LabelToggle.setProperty('class','tool-btn tool-btn-hov edit-head')
        else:
            self.ui.FolderFilter.setParent(None)
            # graceful degradation -
            # button does not appear as clickable
            self.ui.LabelTool.LabelToggle.setCursor(qt_shims.get_QCursor()(qt_shims.get_Qt().ArrowCursor))
            self.ui.LabelTool.LabelToggle.setProperty('class','tool-btn edit-head')
    ...
    def get_plugin_data(self):
        def sort_with_progress(constant, mcounter):
            def custom_sort(item):
                ...
                self.ui.progressBar.updateProgress(finished)
        ...
        self.ui.progressBar.updateProgress(100)
        self.prepareView()
    ...
    def updatePbFunc(self):
        self.sel_dirs = self.ui.FolderFilter.getData()
        self.sel_prfx = self.ui.PrefixFilter.getData()
        self.sel_colr = self.ui.ColorFilter.getSelectedColors()
    ...
    def addMerge(self):
        ...
        merge_name = self.ui.LabelTool.getLabelName()
        if self.env_desc.feat_folders and self.mode_merge == 'folder':
            ida_utils.create_folder(merge_name)
            self.ui.FolderFilter.addItemNew("/{}".format(merge_name))
        elif self.mode_merge == 'prefix':
            self.ui.PrefixFilter.addItemNew(merge_name)
    ...
    def clsMerge(self):
        if self.ui.ResultsView.selectionModel().hasSelection():
            ...
            pfx = self.ui.LabelTool.getLabelName()
    ...
    def toggleModeMerge(self):
        ...
        self.ui.LabelTool.LabelToggle.setText(_translate(btn_caption, btn_caption, None))
        self.ui.LabelTool.LabelEdit.setPlaceholderText(edit_placeholder)
