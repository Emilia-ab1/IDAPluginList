...
SCRIPT_NAME = i18n('API Lib Usage Analysis')
SCRIPT_TYPE = 'custom'
SCRIPT_VIEW = 'tree'
SCRIPT_ARGS = []
...
def get_data(progress_callback=None, env_desc=None, plug_params=None):
    ...
    for i, (addr, module, name, ordi) in enumerate(imps):
        for xref in idautils.XrefsTo(addr):
            ...
            # call_name = ida_shims.get_func_name(func_addr)
            # call_flat = idc.demangle_name(call_name, idc.INF_SHORT_DN)
            # if call_flat:
            #     call_name = call_flat
            # lib_name = call_name[:call_name.find("_")]

            if func_desc and func_desc.flags & idaapi.FUNC_THUNK:
                key_name = "{}_{}".format(module, name)
                report['data'][module].append(ida_shims.start_ea(func_desc))
                report['stat'][module] += 1
