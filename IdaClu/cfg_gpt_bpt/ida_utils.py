...
def graph_down(ea, depth=0, path=dict()):
    path[ea] = depth
    call_instructions = []
...
def recursive_prefix(addr, add_depth=False):
    ...
    if func_addr == idaapi.BADADDR:
        ...
        return
    nodes_xref_down = graph_down(func_addr, depth=0, path=dict())
    addr_set = set()
    if add_depth:
        addr_dict = collections.OrderedDict(sorted(nodes_xref_down.items(), key=lambda item: item[1]))
        addr_set = set(addr_dict.items())
    else:
        addr_set = set(nodes_xref_down.keys())
    return addr_set
