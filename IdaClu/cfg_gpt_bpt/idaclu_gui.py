...
class IdaCluDialog(QWidget):
    def __init__(self, env_desc):
        ...
        self.heads = ['Name', 'Address', 'Size', 'Chunks', 'Nodes', 'Edges', 'Comment', 'Color', 'State']
    ...
    def bindClicks(self):
        ...
        self.ui.wButtonTool.setHandlerBPT(self.setBPT)
        self.ui.wButtonTool.setHandlerGPT(self.setGPT)
        self.ui.wConfigButton.clicked.connect(self.showConfig)
    ...
    def get_plugin_data(self):
        ...
        for dt in sdata:
            for tt in sdata[dt]:
                ...
                bpt = idaapi.bpt_t();
                is_bpt = idaapi.get_bpt(func_addr,bpt)
                is_gpt = idaapi.get_func_cmt(func_desc, 0) == 'chatgpt'
                entry['func_stat'] = [is_bpt, is_gpt]
    ...
    def prepareView(self):
        ...
        self.ui.rvTable.setColumnHidden(self.heads.index('State'), True)
                ...
                for id_group, id_child in self.rec_indx[func_addr]:
                    ...
                    if label_mode == 'prefix':
                        ...
                        if label_norm != folder_src:
                            ...
                            icon = QIcon(':/idaclu/icon_64.png')
                            self.ui.rvTable.model().setData(indx_child, icon, Qt.DecorationRole)
    ...
    def setBPT(self):
        ...
        for func_addr in addr_queue:
            ida_shims.add_bpt(func_addr)
            for id_group, id_child in self.rec_indx[func_addr]:
                id_col = self.heads.index('State')
                indx_group = self.ui.rvTable.model().index(id_group, 0, QtCore.QModelIndex())
                indx_child = self.ui.rvTable.model().index(id_child, id_col, indx_group)
                ...
                is_bpt = idaapi.get_bpt(func_addr,bpt)
                is_gpt = idaapi.get_func_cmt(func_desc, 0) == 'chatgpt'
                new_state = [is_bpt, is_gpt]

                self.ui.rvTable.model().setData(indx_child, new_state, Qt.DecorationRole)
    ...
    def setGPT(self):
        addr_queue = self.getLabelAddrSet(add_depth=True)
        for func_addr, func_depth in sorted(addr_queue, key=lambda x: x[1], reverse=True):
            print(hex(func_addr), func_depth)  # replace with call to chat gpt
            func_desc = idaapi.get_func(func_addr)
            idaapi.set_func_cmt(func_desc, 'chatgpt', 0)
    ...
    def getLabelAddrSet(self, add_depth=False):
        id_col = self.heads.index('Address')
        ...
        if self.is_mode_recursion == True:
            for func_addr in addr_queue:
                if add_depth:
                    func_addr_origin = func_addr[0]
                else:
                    func_addr_origin = func_addr
                addr_calees.update(ida_utils.recursive_prefix(func_addr_origin, add_depth))
    ...
    def showConfig(self):
        ...
        layout = self.ui.hlResultsView

        # self.ui.MainFrame.setParent(None)
        # configFrame = QFrame(self.ui.DialogSplitter)


        #configFrame.setObjectName(u"ConfigFrame")
        #wResultsView = QWidget(configFrame)

        # hlResultsView = QVBoxLayout(wResultsView)
        # hlResultsView.addWidget(wConfigButton)

        if not self.is_config_shown:
            self.ui.stackedWidget.setCurrentWidget(self.ui.wConfigFrame);
            # self.ui.hlResultsView.removeWidget(self.ui.wResultsView)
            # self.ui.wResultsView2.hide()
            # self.ui.hlResultsView.addWidget(self.ui.wConfigButton)
            # self.ui.wResultsView.deleteLater()
            # layout.addWidget(wConfigButton)
            # layout.removeWidget(self.ui.MainFrame)
            # layout.replaceWidget(0, self.ui.wConfigButton)

        else:
            self.ui.stackedWidget.setCurrentWidget(self.ui.wResultsView);
            # self.ui.wResultsView2.show()
            #self.ui.hlResultsView.removeWidget(self.ui.wConfigButton)
            # layout.replaceWidget(0, self.ui.rvTable)
            #layout.addWidget()
            #layout.insertWidget(configFrame)



        self.is_config_shown = not self.is_config_shown
        print('hello')
            ...
            if len(sp_tree[spg_ref]):
                ...
                for sp_fname in sp_tree[spg_ref]:
                    ...
                    with plg_utils.PluginPath(os.path.join(sp_path, spg_ref)):
                        print(os.path.join(sp_path, spg_ref))
                        is_plug_ok = False
                        try:
